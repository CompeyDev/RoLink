  ...
  async checkPremium() {
    if (this.discordBot.isPremium()) {
      ...
      const response = await request({
        uri: `https://registry.RoLink.link/is-premium/${this.server.ownerID}`,
        json: true,
        headers: {
      ...
      if (this.server.systemChannel) {
        try {
          await this.server.systemChannel.send(
            `<@${this.server.ownerID}> An important notice was triggered and **there is no announcement channel configured** and **the server owner will not accept DMs from RoLink**, so it has been posted here:`,
            { embed },
          )
          return
        } catch (e) {}
      ...
      if (options.lastResortChannel) {
        try {
          await options.lastResortChannel.send(
            `<@${this.server.ownerID}> An important notice was triggered and **there is no announcement channel configured**, **the server owner will not accept DMs from RoLink**, and **this server has no systems messages channel**, so it has been posted here as a last resort:`,
            { embed },
          )
        } catch (e) {}
      }
