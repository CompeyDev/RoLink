...
class DiscordMember {
  ...
  async prepareMember() {
    try {
      ...
      if (config.loud)
        console.log(`prepareMember: ${e.message}; ${this.id}; ${this.user}`)
      return false
    }
  }
  ...
  shouldUpdateNickname(currentName) {
    if (this.discordServer.getSetting("nicknameUsers")) {
      return (
        !this.discordServer.nicknames.has(this.id) ||
  ...
  async getNickname(data) {
    ...
    if (this.discordServer.getSetting("nicknameGroup")) {
      const apiRank = await DiscordServer.getRobloxMemberGroups(
        nicknameData.robloxId,
      )

      for (const groups of apiRank) {
        if (
          parseInt(groups.group.id) ===
          parseInt(this.discordServer.getSetting("nicknameGroup"))
        ) {
          const rankMatch = groups.role.name.match(/(.+(?:\]|\)|\}|\|))/)
          nicknameData.groupRank = rankMatch
            ? rankMatch[1]
            : `[${groups.role.name}]`
          break
        }
    ...
    return Util.formatDataString(
      this.discordServer.getSetting("nicknameFormat"),
      nicknameData,
      this.member,
    )
  }
  ...
  async verify(options) {
    ...
    if (options.clearBindingsCache !== false) {
      this.discordServer.cleanupRankBindings(
        options.message ? options.message.channel : undefined,
      )
    }
    ...
    if (options.message) {
      ...
      if (
        this.discordServer.verifyCooldowns.has(this.id) &&
        Date.now() - this.discordServer.verifyCooldowns.get(this.id) < 5000
      ) {
        return {
          status: false,
          nonFatal: true,
          error:
            ":alarm_clock: Please wait at least five seconds before trying to verify again.",
        }
      }
      ...
      status = (action) => {
        ...
        ;(async () => {
          ...
          if (typeof action === "string") {
            ...
            statusMessage.edit(
              `${this.member}, :exclamation:${
                action.error.startsWith(":") ? "" : " "
              }${action.error}`,
            )
          } else if (action.status === true) {
            let welcomeMessage = this.discordServer.getWelcomeMessage(
              action,
              this.member,
            )
            ...
            if (!userRequestSucceeded) {
              welcomeMessage += `\n\n**Warning: An error occured when fetching the latest information from Roblox, ${
                options.skipWelcomeMessage ? "their" : "your"
              } nickname may not be up-to-date!**`
            }

            statusMessage.edit(
              `${options.message.author}, :white_check_mark: ${welcomeMessage}`,
            )
          }
        })()
    ...
    if (this.member.roles.cache.find((role) => role.name === "RoLink Bypass")) {
      return status({
        status: false,
        error:
      ...
      return status({
        ...
        error:
          this.member.guild.ownerID === this.member.id
            ...
            : "\n\nRoLink can't manage this user. Please have a server admin drag RoLink's role above all other roles and ensure RoLink has permission to modify roles in order to fix this problem.",
        nonFatal: true,
      })
    }
    ...
    try {
      ...
      return status({
        status: false,
        error:
          "There was an error while trying to fetch this user's verification data!",
      })
    }
    ...
    if (data.status === "ok") {
      ...
      if (this.discordServer.getSetting("verifiedRole")) {
        ...
        if (
          !this.member.roles.cache.has(role) &&
          this.server.roles.cache.has(role) &&
          ...
          try {
            ...
            return status({
              ...
              nonFatal: true,
              error:
                "There was an error while trying to assign the verified role! Ensure RoLink's role is above it." +
                errorAppend,
            })
          }
        }
        ...
        if (
          ...
          try {
            ...
            return status({
              ...
              nonFatal: true,
              error:
                "There was an error while trying to remove the verified role! Ensure RoLink's role is above it." +
                errorAppend,
            })
          }
        }
      ...
      if (
        ...
        !this.member.roles.cache.find(
          (role) => role.name === "RoLink Nickname Bypass",
        ) &&
        ...
        if (this.member.displayName !== nickname) {
          try {
            ...
            return status({
              ...
              error:
                this.member.guild.ownerID === this.member.id
                  ? "Sorry, Discord doesn't allow bots to change the server owner's nickname. Please manually update your nickname. Or don't, I'm just an error message."
                  : "RoLink doesn't have permission to change that user's nickname." +
                    errorAppend,
            })
          }
        }
      ...
      if (options.announce !== false) {
        this.discordServer.announce(
          "User Verified",
          `<@${this.id}> verified as [${data.robloxUsername}](https://www.roblox.com/users/${data.robloxId}/profile)`,
        )
      }
      ...
      if (this.discordServer.getSetting("groupRankBindings").length > 0) {
        ...
        const promises = []
        for (const binding of this.discordServer.getSetting(
          "groupRankBindings",
        )) {
          // We use a Promise.then here so that they all execute asynchronously.
          promises.push(
            DiscordServer.resolveGroupRankBinding(
              binding,
              data.robloxId,
              data.robloxUsername,
            ).then((state) => {
      ...
      switch (data.errorCode) {
        case 404: {
          ...
          let error = `:wave: You must be new! Please go to ${Util.getVerifyLink(
            this.discordServer.server,
          )} and follow the instructions on the page in order to get verified.`
          ...
          if (options.message) {
            ...
            VerificationAttempts.set(
              this.id,
              VerificationAttempts.get(this.id) + 1,
            )

            if (VerificationAttempts.get(this.id) > 1) {
              error = stripIndents`:question: Looks like you are having trouble verifying your account! Here are some things you can try:

                - Try visiting <https://RoLink.link/verify> in an incognito / private browser window. (It's possible you are signed into the wrong Discord account in your browser)
        ...
        default:
          ...
          return status({
            status: false,
            error:
              "Sorry, it looks like there's something wrong with the verification registry. Please try again later.",
          })
      }
    }
  }
