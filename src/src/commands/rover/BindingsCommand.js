...
module.exports = class BindingsCommand extends Command {
  constructor(client) {
    super(client, {
      name: "bindings",
      properName: "Bindings",
  ...
  async fn(msg) {
    ...
    if (bindingText.length > 0) {
      ...
      msg.reply(
        "No verified or unverified roles are configured. Run `!verifiedrole <role>` or `unverifiedrole <role>` to set them.",
      )
    }
    ...
    for (let binding of this.server.getSetting("groupRankBindings")) {
      ...
      groupBindingsText += `${await this.getRoleName(
        id,
      )} <${id}>\n\`\`\`markdown\n`
