...
const getUnauthorizedMessage = (member, server) =>
  `Sorry, this server isn't authorized to use RoLink Plus.\n\n${
    member.hasPermission(["MANAGE_GUILD"])
      ? "The server owner needs to get plus at <https://RoLink.link/plus>, or you can invite the regular RoLink bot at <https://RoLink.link>."
      : "**This isn't your fault, and there's nothing you can do** - please ask the server staff to update their RoLink Plus subscription."
...
class DiscordBot {
  ...
  initialize() {
    ...
    this.bot.dispatcher.addInhibitor((msg) => {
      ...
      if (this.blacklist[msg.guild.ownerID]) {
        msg.reply(
          "This server is blacklisted because you are banned from the RoLink support server.",
        )
        return "blacklisted"
      }
    })
  ...
  ready() {
    ...
    this.bot.user.setActivity("RoLink.link", { type: "LISTENING" })
  }
  ...
  async message(message) {
    ...
    if (
      ...
      !(
        ...
        message.member.roles.cache.find((role) => role.name === "RoLink Admin")
      )
    ) {
      if (
  ...
  setActivity(text, activityType) {
    ...
    this.bot.user.setActivity(text || "http://eryn.io/RoLink", {
      type: activityType,
    })
  }
