...
module.exports = class NotVerifiedRoleCommand extends Command {
  constructor(client) {
    super(client, {
      name: "unverifiedrole",
      ...
      description:
        "`<Discord Role>` Set the role that unverified users will get, and then lose when they verify. Default none",
  ...
  async fn(msg, args) {
    ...
    if (role) {
      ...
      if (this.server.isRoleInUse(role.id)) {
        msg.reply(
          `That role is already in use. (verified role, not verified role, or from a group binding). Run \`${msg.guild.commandPrefix}bindings\` to see all role bindings.`,
        )
      } else {
      ...
      msg.reply(
        "Cleared removed verification role, users will no longer be removed from it on verification.",
      )
    }
  }
}
